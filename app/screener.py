...
# Sector tags loaded from app/data/sectors.json (generated by build_sectors.py,
# which lives outside the repo). Curated GICS sectors keep us off yfinance for
# this field at runtime and provide stable ETF categories (Broad Market /
# Fixed Income / Commodity / International / Volatility).
_SECTORS_PATH = Path(__file__).parent / "data" / "sectors.json"

def _load_sectors() -> dict[str, dict]:
    try:
        with open(_SECTORS_PATH) as f:
            data = json.load(f)
        logger.info(f"Loaded sectors.json: {len(data)} ticker tags")
        return data
    except (FileNotFoundError, json.JSONDecodeError) as exc:
        logger.warning(f"Could not load sectors.json ({exc}); sector lookups will return Unknown")
        return {}
...
def run_screening_pipeline(
    ...
    for _, row in final.iterrows():
        shortlist.append(ScreenedAsset(
            ...
            sector=(_SECTORS.get(row["ticker"], {}).get("sector")
